export const github=<i className="fa-brands fa-github"></i>;
export const star=<i className="fa-solid fa-star"></i>;
export const trash=<i className="fa-solid fa-trash"></i>;
export const edit=<i className="fa-solid fa-pen-to-square"></i>;
export const check=<i className="fa-solid fa-check"></i>;
export const times=<i className="fa-solid fa-timer"></i>;
export const plus=<i className="fa-solid fa-plus"></i>;
export const sun=<i className="fa-solid fa-sun"></i>;
export const moon=<i className="fa-solid fa-sun"></i>;
export const badge=<i className="fa-solid fa-badge"></i>;
export const mail=<i className="fa-solid fa-mailbox"></i>;
export const profile=<i className="fa-solid fa-user"></i>;
export const home=<i className="fa-solid fa-house"></i>;